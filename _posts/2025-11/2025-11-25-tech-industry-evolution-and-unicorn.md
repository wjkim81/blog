---
layout: post
title: "From Handsome Horses to Actual Unicorns: Why AI Expertise Is Different"
date: 2025-11-26
published: true
categories: [AI]
...
# Part 1: From Handsome Horses to Actual Unicorns
...
**잘생긴 말에서 진짜 유니콘까지: 왜 AI 전문성은 ‘다른 차원’인가**
...
## 1. 실제로 올라온 채용 공고들 (원래는 영문)

지난주에 본 실제 공고.

**필수 요건:**

* 솔루션 아키텍처 경력 5년 이상 (그중 AI/ML 3년 이상)
* 전통 ML / 딥러닝 / 생성형 AI에 대한 깊은 이해
* 스케일 가능하고 프로덕션급 AI 시스템 설계 경험
* AWS/Azure/GCP 클라우드 아키텍처 전문성
* 데이터 아키텍처 / 파이프라인 / 데이터 거버넌스 전문성
* MLOps / 모델 배포 / 모니터링 경험
* 기술팀 리딩 및 이해관계자 조율 경험

**우대 사항:**

* CS/AI/Data Science 석·박사
* 엔터프라이즈 AI 플랫폼 경험
* AWS/Azure/GCP 자격증
* AI 윤리, Responsible AI, 규제 준수 경험
* 다양한 산업군 경험
* 엣지 AI, IoT 아키텍처 이해
* 대화형 AI, NLP, CV 경험
* AI 비용 최적화 이해
...
```
한 명이 동시에 다음 역할을 수행해야 함:
├─ 솔루션 아키텍트 (5년)
├─ AI/ML 전문가
├─ 딥러닝 전문가
├─ 생성형 AI 전문가
├─ 클라우드 아키텍트 (AWS+Azure+GCP)
├─ 데이터 아키텍트
├─ 데이터 엔지니어
├─ 데이터 거버넌스 전문가
├─ MLOps 엔지니어
...
├─ NLP 전문가
├─ Computer Vision 전문가
├─ Conversational AI 전문가
└─ 9개 더 추가

총 20개 전문성을 한 사람이 다 가지는 구조.
```
...
* AI 관련 경력 5년
* 파이썬으로 AI 모델 개발 능숙
* React로 UI를 스케일링하며 개발한 경험
* API 설계/구현/배포에 숙련
* TensorFlow, PyTorch, scikit-learn 실무
* **10년 이상의 IT 전체 분야 경험 (IT 아키텍처, 인프라, DB, 클라우드, 소프트웨어 아키텍처, 비즈니스 분석, DevOps, 프로젝트·프로덕트 관리)**

그리고 마지막에 이렇게 적혀 있다.

> “면제(exempt) 직군이므로 주당 40시간을 초과하여 근무할 수 있습니다.”

AI 아키텍트인데 React 프런트엔드도 잘해야 하고, 10년 경력도 있어야 하고,
심지어 야근도 가능해야 함.
...
문제는 기업들이 **많이 요구해서**가 아니다.

문제는 지금 기업들이 요구하는 것이
**기술적으로 ‘가능한 조합’이 아니라는 점**이다.

예전 기술 파동에서 “유니콘 채용공고”에서도 비슷한 사례가 많았지만
이번에는 “좀 과한 기대치”인 것 같다.

사실상 **불가능한 조합**을 요구하고 있다.

2010년 이후 예전 사례부터 보자.

---

## Act 1: 클라우드 엔지니어 “유니콘” 시절 (2010–2015)

2010년대 초, 클라우드 아키텍트는 유니콘 취급을 받았다.
...
기존 시스템 엔지니어가 6~12개월만 집중하면
클라우드 업무로 갈알 탈 수 있는 구조였다.
...
지금 생각해 보면 **그저 “잘생긴 말”이었다.** 🐴

---

## Act 2: 데이터 사이언티스트 “유니콘” 시절 (2012–2020)

2012년 HBR이 이렇게 말했다:

> “데이터 사이언티스트는 21세기 가장 섹시한 직업.”
...
데이터 사이언티스트도 유니콘이 아니었다.

**“잘생긴 말 + 예쁜 안장.** 🐴✨

---

## Act 3: AI 전문가 “유니콘” (2023–현재)
...
```python
create_server(config) → 항상 동일한 결과
```

AI 시스템:

```python
llm.complete(document) → 호출할 때마다 결과 다름
```

디버깅 방식 자체가 다르다.

* 결정론: 원인 찾고 고치면 끝
* 확률론: “왜 이런 상황은 되고 저 상황은 안 되지?”
...
클라우드:
AWS EC2 (2006) → 지금도 유효
Kubernetes (2014) → 여전히 표준
Docker (2013) → 여전히 표준

AI:
BERT 파인튜닝 (2019) → 사실상 구식
2023년 RAG 패턴 → 이미 세대교체
프롬프트 패턴 → 매달 변화
“베스트 프랙티스” → 계속 변함
```

AI는
**배우는 동안에도 항상 새로운 게 나온다.**

---

요약하면:

* Cloud → Data Science: **스킬 추가 (확장)**
* Data Science → AI Expert: **사고방식 전환 (변형)**

그리고 AI 기술을 익히기 위해서는 
6~12개월 집중 학습으로는 해결되지 않는다.
...
AI 시스템 개발 경험,
클라우드/인프라도 공부,
AI 논문도 연구하다 보니 계속해서 읽는다.

사실 캐리어를 조금은 의도적으로 **너비와 깊이**를 동시에 쌓아오도록 노력한 편이라고 생각한다.

하지만 위의 JD들을 보면,
내 개인적으로도 요구하는 수준의 **절반 정도**밖에 충족하지 못한다고 느낀다.

애초에 **요구사항이 현실적이지 않은 것**을 요구하기 때문이다.
...
여기까지는 
“AI가 기술 스택이 많은 좀 더 어려운 기술이라 그렇다”고 생각할 수도 있다.

하지만, AI 전문성이 **유독 불가능한 이유**는 또 따로 있다.

2022년에 ChatGPT에 의해서 AI, 특히 LLM 필드 전체가 **리셋**되었다.

* 모두가 초보자가 되었다.
* 심지어 기존 “전문가”들도 다시 처음부터 시작해야 했다.

이러한 부분에 대해서는 다음 글에 대해서 다뤄 볼 예정이다.

---

**다음 글: “Part 2 — 모든 것이 리셋된 시대: ‘LLM 5년 경력’이 수학적으로 불가능한 이유”**
