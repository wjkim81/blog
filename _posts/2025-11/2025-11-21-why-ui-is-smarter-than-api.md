...
flowchart TD
    ...
    %% ----------------- Tutorial RAG -----------------
    subgraph T[Tutorial RAG]
        A1(Documents)
        A2[Embed Text]
        A3[Store in Vector DB]
        A4[Retrieve Top-K]
        A5[LLM]
        A6(Output)
    ...
    %% ----------------- Real ChatGPT -----------------
    subgraph C[Real ChatGPT / Claude Orchestration]

        %% File Processing
        subgraph F[File Processing (10 steps)]
            F1(Type Detection)
            F2(AST / Section Parsing)
            F3(Intelligent Chunking)
            F4(Metadata Extraction)
            F5(Cross-file Linking)
            F6(Knowledge Graph)
            F7(Multi-index Embeddings)
            F8(Topic Classification)
            F9(Safety Scan)
            F10(Storage in Multi-index DB)
        end

        %% Query Processing
        subgraph Q[Query Processing (15 steps)]
            Q1(Intent Classification)
            Q2(Task Type Detection)
            Q3(Retrieval Strategy Select)
            Q4(Multi-index Retrieval)
            Q5(Semantic Search)
            Q6(Keyword Search)
            Q7(Graph-based Search)
            Q8(Reranking)
            Q9(Depth vs Breadth Balancing)
            Q10(Filter Irrelevant Chunks)
            Q11(Conversation State Lookup)
            Q12(Memory Summaries)
            Q13(Context Fitting)
            Q14(Instruction Selection)
            Q15(Final Chunk Ordering)
        end

        %% Context Construction
        subgraph P[Context Engineering (5 steps)]
            P1(System Prompt Injection)
            P2(Metadata Injection)
            P3(File Wrappers)
            P4(Structured Prompt Formatting)
            P5(Hidden Model Instructions)
        end
        ...
        %% Post-Processing
        subgraph O[Post-Processing (5 steps)]
            O1(Add Citations)
            O2(Style/Format Correction)
            O3(Safety Filter)
            O4(Hallucination Check)
            O5(Final Output Formatting)
        end
    end
